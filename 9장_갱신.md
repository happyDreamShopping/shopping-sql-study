...
- UPDATE 성능 개선 방법 [SQL1](http://sqlfiddle.com/#!17/96de7/5)
  1. 반복계를 피하고 가급적 인덱스를 사용하자 (쿼리 플랜의 중요성)
...
>
> 총 3번의 scan이 이뤄지지만 인덱스를 타게 됨으로 성능의 향상을 기대



2. 레코드에서 필드로 갱신하는 방법 [SQL2](http://sqlfiddle.com/#!17/d1e84/12)
...
- 한번에 갱신 -> 성능 향상
- 과목이 늘어나도 서브쿼리의 개수가 늘어나지 않아 향후 성능 악화의 염려가 적음
...
3-1 ) 필드에서 레코드로 변경하는 방법 [SQL3](http://sqlfiddle.com/#!17/eed8c/8)
...
>
> A001 -> (100, 58, 90), B002 -> 77, 60, NULL ... 가져와 한번에 처리됨.
...
- Stock2 (현재 종가를 이전 종가와 비교해서 올랐다면 등 주요 '상승', 내렸다면 '하락', 그대로라면 '-' 를 trend에 표기)
...
- INSERT와 UPDATE 어떤게 더 좋을까?
  ...
  - INSERT SELECT의 경우
    ...
    - 단점 : 저장소의 용량을 2배 이상 소비한다
      ...
      1. 장점 : VIEW를 사용하면 용량을 절약할 수 있고 정보를 항상 최신으로 유지할 수 있다.
      2. 단점 : VIEW에 접근이 발생할 때마다 복잡한 연산이 수행되므로 VIEW에 접근하는 쿼리의 성능이 낮아진다. 
      3. 결국 성능과 동기성의 트레이드오프.
